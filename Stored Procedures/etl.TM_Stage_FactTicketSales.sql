...
LEFT OUTER JOIN etl.vw_DimArena (NOLOCK) dArena ON dEvent.DimArenaId = dArena.DimArenaId
	AND dArena.DimArenaId > 0 AND dArena.ETL__IsDeleted = 0

LEFT OUTER JOIN etl.vw_DimSeason (NOLOCK) dSeason ON dEvent.DimSeasonId = dSeason.DimSeasonId
	AND dSeason.DimSeasonId > 0 AND dSeason.ETL__IsDeleted = 0
