...
CREATE   VIEW [etl].[vw_Load_DimCustomer_CRM_Account] AS 
...
--base set
		...
		'MSCRM' AS [SourceDB]
		, 'CRM_Account' AS [SourceSystem]
		, 1 AS [SourceSystemPriority]
		, CAST(c.accountid AS NVARCHAR(50)) AS [SSID]
		...
		/*AddressPrimary*/
			...
			/*Extended Attributes*/
			, CAST(c.ownerid AS NVARCHAR(50)) AS [ExtAttribute1] --nvarchar(100)
			, null AS [ExtAttribute2] 
			, c.primarycontactid AS[ExtAttribute4] 
			, NULL AS[ExtAttribute3] 
			, NULL AS[ExtAttribute5] 
			, NULL AS[ExtAttribute6] 
			, c.accountid AS[ExtAttribute7] 
			...
			, TRY_CAST (c.koreps_lastcontacted AS DATE) AS [ExtAttribute21] --datetime
...
--			select top 100 *	
			...
			FROM oilers_reporting.Prodcopy.Account c
			--FROM oilers_reporting.Prodcopy.Account_old c
