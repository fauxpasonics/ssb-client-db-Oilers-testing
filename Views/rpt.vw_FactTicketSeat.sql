...
WITH CTE_Seats (DimSeasonId, DimArenaId, DimEventId, SectionName, RowName, Seat, DimSeatId, IsSaleable, IsSold)
AS
(
	SELECT
		fi.DimSeasonId,
...
SELECT
	...
	, dst.SSID_row_id
	, dst.SSID_seat
	, vsc.SeatCenter_X, vsc.SeatCenter_Y
	, CAST(CASE WHEN ((ls.IsSold = 0 And ls.IsSaleable = 1) OR (rs.IsSold = 0 AND rs.IsSaleable = 1)) AND fi.IsSold = 0 AND fi.IsSaleable = 1 THEN 1 ELSE 0 END AS BIT) AS IsPair
...
LEFT OUTER JOIN dbo.FactTicketSales (NOLOCK) fts ON fi.FactTicketSalesId = fts.FactTicketSalesId
    ...
    LEFT OUTER JOIN dbo.HeatMap_SeatCoordinates vsc ON dst.RowName = vsc.row AND dst.Seat = vsc.seat AND dst.SectionName = vsc.Section
--WHERE fi.DimSeasonId IN (48,49,50,52,53, 61)

LEFT OUTER JOIN CTE_Seats ls
	ON  fi.DimSeasonId = ls.DimSeasonId
	AND fi.DimEventId = ls.DimEventId
	AND dst.SectionName = ls.SectionName
	AND dst.RowName = ls.RowName
	AND dst.Seat = (ls.Seat + 1)
LEFT OUTER JOIN CTE_Seats rs
	ON  fi.DimSeasonId = rs.DimSeasonId
	AND fi.DimEventId = rs.DimEventId
	AND dst.SectionName = rs.SectionName
	AND dst.RowName = rs.RowName
	AND dst.Seat = (rs.Seat - 1)
