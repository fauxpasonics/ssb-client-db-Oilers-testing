...
AS
	...
	SELECT    ssbid.SSB_CRMSYSTEM_CONTACT_ID
      ...
      FROM      dbo.DimCustomer dimcustomer WITH ( NOLOCK )
				...
				LEFT JOIN (SELECT SSB_CRMSYSTEM_CONTACT_ID, CAST(contactid AS NVARCHAR(50)) AS contactid,owneridname 
							FROM Oilers_Reporting.[Prodcopy].[contact] (NOLOCK) contact
							INNER JOIN  (
								SELECT SSB_CRMSYSTEM_CONTACT_ID, SSID
								, ROW_NUMBER() OVER(PARTITION BY SSB_CRMSYSTEM_CONTACT_ID ORDER BY updateddate DESC, createddate) xRank 
								FROM [dbo].[vwDimCustomer_ModAcctId] (NOLOCK) dc
								WHERE dc.SourceSystem= 'CRM_Contact'
										 ) x
					ON contact.contactid = x.SSID AND x.xRank = '1' 
							) crm ON crm.SSB_CRMSYSTEM_CONTACT_ID = ssbid.SSB_CRMSYSTEM_CONTACT_ID
